...
<section class="content">
  ...
  <input type="text" id="ex_input">
        ...
        <table class='type02' style="text-align:center; margin:2% 0 0 0;">
            ...
            <?php
            $query = "select orderlist.order_number, orderlist.menu_name, orderlist.member_id, orderlist.order_quantity, menu.menu_price, orderlist.order_date, member.member_name, member.phone_number, member.company_name, company.company_phonenumber from orderlist, 
                        member, menu, company where orderlist.member_id = member.member_id and orderlist.menu_no = menu.menu_no and member.company_name = company.company_name";
            $result = oci_parse($conn, $query);
            oci_execute($result);
            for($i=0;$array = oci_fetch_array($result);$i++)
            {
